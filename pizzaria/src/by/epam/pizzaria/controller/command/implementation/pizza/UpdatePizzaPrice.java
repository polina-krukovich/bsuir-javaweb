...
public class UpdatePizzaPrice implements Command {
    /**
     * Execute Update Pizza Price command
    ...
    public Response execute(Request request) {
        ...
        try {
            pizzaService.updatePizzaPrice(pizzaRequest.getId(), pizzaRequest.getPrice());
        } catch (ServiceException e) {
            pizzaResponse.setErrorMessage(e.getMessage());
            pizzaResponse.setStatus(CommandStatus.ERROR);
            return pizzaResponse;
        }
        pizzaResponse.setSuccessMessage("Successfully updated pizza price");
