...
public class UpdatePizzaName implements Command {
    /**
     * Execute Update Pizza Name command
    ...
    public Response execute(Request request) {
        ...
        try {
            pizzaService.updatePizzaName(pizzaRequest.getId(), pizzaRequest.getName());
        } catch (ServiceException e) {
            pizzaResponse.setErrorMessage(e.getMessage());
            pizzaResponse.setStatus(CommandStatus.ERROR);
            return pizzaResponse;
        }
        pizzaResponse.setSuccessMessage("Successfully updated pizza name");
